		...
		for (const fastgltf::Mesh & gltf_mesh: gltf.meshes)
		{
			...
			for (const fastgltf::Primitive & gltf_primitive: gltf_mesh.primitives)
			{
				...
				fastgltf::iterateAccessor<glm::vec3>(gltf, gltf.accessors.at(gltf_primitive.findAttribute("POSITION")->accessorIndex), [&](glm::vec3 position) {
					aabb_min.x = std::min(aabb_min.x, position.x);
					aabb_min.y = std::min(aabb_min.y, position.y);
					aabb_min.z = std::min(aabb_min.z, position.z);
					aabb_max.x = std::max(aabb_max.x, position.x);
					aabb_max.y = std::max(aabb_max.y, position.y);
					aabb_max.z = std::max(aabb_max.z, position.z);
				});
