...
struct frustum
{
	...
	frustum(const glm::mat4 & proj)
	{
		 ...
		 *
		 * dot(plane[0], [x,y,z,1]) > 0 <=> x' < w'  <=> w' - x' > 0 <=> planes[0] = row3-row0
		 * dot(plane[1], [x,y,z,1]) > 0 <=> x' > -w' <=> x' + w' > 0 <=> planes[1] = row0+row3
		 * dot(plane[2], [x,y,z,1]) > 0 <=> y' < w'  <=> w' - y' > 0 <=> planes[2] = row3-row1
		 * dot(plane[3], [x,y,z,1]) > 0 <=> y' > -w' <=> y' + w' > 0 <=> planes[3] = row1+row3
		 * dot(plane[4], [x,y,z,1]) > 0 <=> z' < w'  <=> w' - z' > 0 <=> planes[4] = row3-row2
		 *
		 * See Gribb & Hartmann
		 */
...
// Return true if the AABB is outside the frustum
...
{
	// TODO https://iquilezles.org/articles/frustumcorrect/
	for (const glm::vec4 & plane: fru.planes)
	{
		int out = 0;
		out += glm::dot(plane, model * glm::vec4(aabb_min.x, aabb_min.y, aabb_min.z, 1)) < 0;
		out += glm::dot(plane, model * glm::vec4(aabb_min.x, aabb_min.y, aabb_max.z, 1)) < 0;
		out += glm::dot(plane, model * glm::vec4(aabb_min.x, aabb_max.y, aabb_min.z, 1)) < 0;
		out += glm::dot(plane, model * glm::vec4(aabb_min.x, aabb_max.y, aabb_max.z, 1)) < 0;
		out += glm::dot(plane, model * glm::vec4(aabb_max.x, aabb_min.y, aabb_min.z, 1)) < 0;
		out += glm::dot(plane, model * glm::vec4(aabb_max.x, aabb_min.y, aabb_max.z, 1)) < 0;
		out += glm::dot(plane, model * glm::vec4(aabb_max.x, aabb_max.y, aabb_min.z, 1)) < 0;
		out += glm::dot(plane, model * glm::vec4(aabb_max.x, aabb_max.y, aabb_max.z, 1)) < 0;
        ...
        bool render_debug_draws)
{
	...
	for (auto && [entity, node]: scene.view<components::node>().each())
	{
		...
		for (renderer::primitive & primitive: node.mesh->primitives)
		{
			...
			glm::vec3 center = 0.5f * (primitive.aabb_min + primitive.aabb_max);
			...
			if (node.joints.empty())
			{
				...
				for (const auto & fru: frusta)
					visible = visible or not frustum_cull(fru, node.transform_to_root, primitive.aabb_min, primitive.aabb_max);
